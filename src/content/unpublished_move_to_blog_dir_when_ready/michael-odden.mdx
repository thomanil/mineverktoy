---
title: 'Michael Odden'
description: 'Selvstendig utvikler'
pubDate: 'Sep 26 2023'
heroImage: '/images/michael-odden/portrait.jpeg'
---

import Question1 from '../../components/Question1.astro';

import Question2 from '../../components/Question2.astro';

import Question3 from '../../components/Question3.astro';

import Question4 from '../../components/Question4.astro';




<Question1/>

Heisann! Michael her. Så hyggelig å få kunne snakke litt om meg og mitt! Dette kan bli langt, så beklager på forhånd.

Av yrke så er jeg utvikler, og for tiden driver jeg som selvstendig konsulent.

Av utdanning har jeg fagbrev i IKT-driftsfag far 2005, og en bachelor fra (den gang) Høgskolen i Buskerud innen dataingeniør-linjen "Simulering og spillutvikling" fra 2011. Mesteparten av mitt yrkesaktive liv siden har jeg jobbet med safetykritiske sanntidssystemer, men har vært innom et knippe ulike teknologier og domener; web, desktop, embedded, spill, forsvar, maritimt, statlig... Men det har vært lite metallnær utvikling i oppdragene de senere årene, så det er en kløe jeg enn så lenge fortsetter å stille på andre plan.

I oppdraget jeg er i nå så hjelper jeg primært kunden (middels stor statlig institusjon) med å bygge opp en overvåkningsløsning for deres IT-infrastruktur og applikasjonsportefølje.

I regi av eget selskap så går det i utvikling av spillkonsepter, nytteverktøy - ihvertfall for meg! - og sære one-offs / nisjeapplikasjoner:

* https://github.com/michaelo/prosit
* https://github.com/michaelo/daya
* https://github.com/michaelo/sapt

Det kan være greit å nevne at spesifikke særheter ved meg innen fagfeltet og dens utøvelse er at jeg er spesielt glad i kontroll, enkelhet og ytelse.

<Question2/>

Generelt er min filosofi at jeg ønsker å være kapabel til å gjøre en størst mulig mengde jobb på ethvert vanilla PC-oppsett. Dette medfører at jeg aldri har gått til noen ekstremiteter hva gjelder produktivitets-grep og fryktelig sære konfigurasjoner. Jeg liker generelt sett singel-skjerm og QWERTY-tastatur.

Allikevel har jeg jo akkumulert en del ting over tid... I praksis så har jeg 3 oppsett det kan være kjekt å skille på;

* Hjemmekontor-oppsettet som primært er koblet opp mot laptopen jeg bruker i det gjeldende oppdraget
* Stasjonær PC med litt mer guffe som brukes til å teste/jobbe med ting som er mer ressurskrevende
* Frittstående laptop brukt til storparten av intern-utvikling

Videre så tar jeg meg den kunstneriske friheten å gå for all hard-vare mer enn kun elektronisk maskinvare, og på den måten inkludere noen analoge aspekter av oppsettet mitt også. (Jeg sa jo at dette kunne bli langt...)

Først av alt; hva jeg _strengt tatt_ må ha for å kunne trives:

* Laptop - jeg har de siste ~15 årene tydd til MacBook Pro's. Men entusiasmen der har begynt å dvale. Jeg er egentlig veldig gira på en unnskyldning for å kunne kjøpe meg en [Framework laptop](https://frame.work/products/laptop16-diy-amd-7040).
* Notatblokk (helst svakt rutet) og blypenn (0.5mm / 0.7mm)
* Et ålreit headset - arbeidshesten over lang tid har vært noen Beyer Dynamics DT770, og de siste årene komplementert av noen AirPods pro.

Alt annet jeg har videre her er kun mindre forbedringer, nice-to-haves osv.

Vel! La oss dykke ned i den faktiske materien her.

**"Hjemmekontoret"**

<img src='/images/michael-odden/oppsett_hjemmekontor.jpeg' alt="Hjemmekontoret"/>

* PC-laptop: something-something Windows-PC
* Skjerm; en 28" 4k av noe slag. La meg sjekke produktkoden her... [Asus VP28UQGL](https://www.asus.com/displays-desktops/monitors/gaming/vp28uqgl/)
* [Planck EZ](https://blog.zsa.io/2307-goodbye-planck-ez/) tastatur - kan ikke linke direkte da det har gått ut av produksjon.
* [Mackie Big Knob](https://mackie.com/intl/products/studio-monitoring/big-knob-series/big_knob_passive.html)
* [KRK RP5 høyttalere](https://www.krkmusic.com/Studio-Monitors/ROKIT-5-G4)
* [Røde NT-USB Mini](https://rode.com/en/microphones/usb/nt-usb-mini)
* Hev/senk skrivebord 120cm\*60cm

**"Compute-heavy"**

<img src="/images/michael-odden/oppsett_compute.jpeg" alt="Den kraftige maskinen"/>

* Stasjonær PC: AMD Ryzen 9 3900X, RTX 3070, 32GB DDR4 3200MHz
* Skjerm; 24" [AOC Q24P2Q](https://aoc.com/uk/products/monitors/q24p2q)
* [Wacom Cintiq 16" tegnebrett](https://www.wacom.com/en-us/products/pen-displays/wacom-cintiq)
* [Mackie Big Knob](https://mackie.com/intl/products/studio-monitoring/big-knob-series/big_knob_passive.html)
* [Genelec G One høyttalere](https://www.genelec.com/g-one)


**Frittstående laptop**

<img src='/images/michael-odden/oppsett_laptop.jpeg' alt="Laptopen"/>

* MacBook Pro (mid-2018)

**Felles/flytende ekstrautstyr**
* [reMarkable 2](https://remarkable.com/store/remarkable-2)
* Notatblokk og blypenn (.5 og .7)
* Headset: [Beyer Dynamic DT770 80Ohm](https://global.beyerdynamic.com/dt-770-pro.html)
* Headset: [Focal Listen](https://www.focal.com/en/pro-audio/monitoring-speakers/professional-headphones/listen-professional)
* Headset: [AirPods Pro (1. gen)](https://www.apple.com/no/airpods-pro/)


**Diverse annet jeg har i skuffen eller hengende rundt omkring**

<img src='/images/michael-odden/kaffe.jpeg' alt="Kafferiggen"/>

* Kaffe-stasj
* [2x Jetson Nano](https://developer.nvidia.com/embedded/jetson-nano-developer-kit)
* [5" Touch-skjerm](https://www.seeedstudio.com/display.html)
* [Shure SM7B](https://www.shure.com/en-US/products/microphones/sm7b)
* Whiteboard 120cm\*90cm









<Question3/>

**Operativsystem: Primær-OSer: macOS, Windows 10, NixOS**

Jeg har så langt vippet i favør macOS, men må si at jeg setter pris på mye ved NixOS. Skal ikke skryte på meg å være på toppen av alt der - men den har gjort det overkommelig å spesifisere opp et rimelig ressurseffektivt miljø hvor jeg står fritt til å fokusere på det som betyr noe.

**Editorer: vscode, neovim, nano**

Ofte i løpet av dagen, og ihvertfall igjennom uka så beveger jeg meg innom et godt sett ulike teknologier, så det er viktigere for meg å ha en konsistent opplevelse heller enn å ha supertett integrasjon med hva-nå-enn. Multi-cursor og god regex søk/erstatt er mine venner.

Jeg har i en periode brukt vscode som utgangspunkt. Kanskje mest av latskap. Men, blodtrykket mitt har ikke godt av dette lenger, så jeg er på aktivt søkende etter en vei vekk fra den. En OK konfigurert neovim (tester ['nvchad'](https://github.com/NvChad/NvChad)-distroen nå) ser så langt ut til å være en grei vei. Tiden vil vise!

**Terminal m. tilbehør**

Jeg er stor fan av terminal. Det er noe av det som gjør at jeg kan gjøre mye av jobben min effektivt på tvers av maskiner, miljøer, nettverksgrenser og teknologier. Ofte kan jeg leve med standard-emulatorene til OSene jeg er i, og ellers har jeg [Alacritty](https://github.com/alacritty/alacritty) på lur når jeg trenger en litt mer effektiv løsning. Jeg har dessverre opplevd en del ustabilitet med Alacritty, så det er ikke udelt elsk der.

Men, viktigere er vel _hva_ jeg kjører i terminalen, og noen gjengangere hvor enn jeg beveger meg er da: [tmux](https://github.com/tmux/tmux/wiki), [watchexec](https://github.com/watchexec/watchexec), [hyperfine](https://github.com/sharkdp/hyperfine) og [prosit](https://github.com/michaelo/prosit), og iblant også [sapt](https://github.com/michaelo/sapt).

**Språk / compilere**

I disse dager så er det en fordeling mellom C#, Python + diverse web-relatert på ene siden (oppdrag) og zig og C på andre siden (interne prosjekter).

**Notater / nytteverktøy**

Jeg brukte lenge Evernote, men har en god tid vært på søken etter en bedre ytende, mindre forstyrrende og mer stabil løsning, som i tillegg lar meg selv ha full kontroll over mine notater.

I en periode kjørte jeg standard editor med Markdown-forhåndsvisning mot et dedikert git-repo, før jeg så fant [Obsidian](https://obsidian.md/) som bare gjorde det mye bedre. Hadde de bare hatt native applikasjoner så hadde den vært så godt som perfekt!

<Question4/>

Oi. Det er et godt spørsmål. Jeg har vel ikke egentlig noe klart fastsatt mål her, og justerer bare litt og litt etterhvert som jeg finner muligheter. Om noe kunne tenkt meg større bordplass, med dedikert plass til tegnebrett mm. Kanskje noe ala slik arkitekt-pult med skråstilt arbeidsflate?

Skulle vi tatt det lenger så hadde det selvsagt ikke vært å forakte å hatt en skjermet hytte i skogen. Kanskje ved en innsjø til og med.